...
fn check_messaging(src: &str) -> Vec<String> {
    use loom::checker::{MessagingChecker, LoomChecker};
...
"#);
    let mp = module.items.iter().find_map(|i| {
        if let Item::MessagingPrimitive(m) = i { Some(m) } else { None }
...
"#);
    assert!(out.contains("MarketFeedSource"), "should emit Source trait: {}", out);
    assert!(out.contains("fn request("), "should emit request (backpressure): {}", out);
    assert!(out.contains("fn poll_next("), "should emit poll_next: {}", out);
...
"#);
    assert!(out.contains("PriceStreamSink"), "should emit Sink trait: {}", out);
    assert!(out.contains("fn on_next("), "should emit on_next: {}", out);
    assert!(out.contains("fn on_complete("), "should emit on_complete: {}", out);
    assert!(out.contains("fn on_error("), "should emit on_error: {}", out);
}
...
"#);
    assert!(out.contains("Reactive Streams"), "should attribute Reactive Streams spec: {}", out);
}
...
"#);
    assert!(out.contains("JobQueueSender"), "should emit Sender trait: {}", out);
...
"#);
    assert!(out.contains("TaskQueueReceiver"), "should emit Receiver trait: {}", out);
...
fn checker_rejects_exactly_once_with_at_most_once() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(!errors.is_empty(), "should reject exactly-once + at-most-once");
    assert!(
        errors.iter().any(|e| e.contains("exactly-once") && e.contains("at-most-once")),
        "error should mention both guarantees: {:?}", errors
    );
}
...
fn checker_rejects_exactly_once_with_at_least_once() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(!errors.is_empty(), "should reject exactly-once + at-least-once");
}
...
fn checker_accepts_at_least_once_without_conflict() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(errors.is_empty(), "at-least-once + ordered should be valid: {:?}", errors);
}
...
fn checker_rejects_stream_with_exactly_once() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(!errors.is_empty(), "stream + exactly-once should be rejected");
    assert!(
        errors.iter().any(|e| e.contains("stream") && e.contains("exactly-once")),
        "error should mention both stream and exactly-once: {:?}", errors
    );
}
...
fn checker_accepts_stream_with_at_least_once() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(errors.is_empty(), "stream + at-least-once should be valid: {:?}", errors);
}
...
fn checker_rejects_request_response_without_timeout_mandatory() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(!errors.is_empty(), "request_response without timeout:mandatory should warn");
    assert!(
        errors.iter().any(|e| e.contains("timeout")),
        "error should mention timeout: {:?}", errors
    );
}
...
fn checker_accepts_request_response_with_timeout_mandatory() {
    ...
    let errors: Vec<String> = MessagingChecker::new()
        ...
        .map(|e| e.to_string())
        .collect();
    assert!(errors.is_empty(), "request_response with timeout:mandatory should pass: {:?}", errors);
}
...
"#);
    assert!(out.contains("OrderServiceClient"), "should emit Client trait: {}", out);
...
"#);
    assert!(out.contains("EventBusSubscriber"), "should emit Subscriber trait: {}", out);
    assert!(out.contains("EventBusBus"), "should emit Bus struct: {}", out);
}
...
"#);
    assert!(out.contains("WorkQueueProducer"), "should emit Producer trait: {}", out);
    assert!(out.contains("WorkQueueConsumer"), "should emit Consumer trait: {}", out);
}
