...
fn kani_harness_emitted_for_fn_with_require() {
    ...
    assert!(code.contains("#[cfg(kani)]"), "missing #[cfg(kani)]:\n{code}");
    assert!(code.contains("#[kani::proof]"), "missing #[kani::proof]:\n{code}");
}
...
fn kani_harness_emitted_for_fn_with_ensure() {
    ...
    assert!(code.contains("#[cfg(kani)]"), "missing #[cfg(kani)]:\n{code}");
    assert!(code.contains("kani::assert"), "missing kani::assert:\n{code}");
}
...
fn kani_harness_uses_kani_any_for_inputs() {
    ...
    assert!(code.contains("kani::any()"), "expected kani::any() for symbolic input:\n{code}");
}
...
fn kani_harness_uses_kani_assume_for_requires() {
    ...
    assert!(code.contains("kani::assume"), "expected kani::assume for require:\n{code}");
}
...
fn kani_harness_uses_kani_assert_for_ensures() {
    ...
    assert!(code.contains("kani::assert"), "expected kani::assert for ensure:\n{code}");
}
...
fn kani_harness_uses_rust_and_operator() {
    ...
    assert!(!code.contains("kani::assume(x > 0 and"), "kani assume must use &&:\n{code}");
}
...
fn v2_gate_add_two_positives_harness_complete() {
    ...
    assert!(code.contains("#[cfg(kani)]"), "V2 gate: #[cfg(kani)]:\n{code}");
    assert!(code.contains("#[kani::proof]"), "V2 gate: #[kani::proof]:\n{code}");
    assert!(code.contains("kani::any()"), "V2 gate: kani::any():\n{code}");
    assert!(code.contains("kani::assume"), "V2 gate: kani::assume:\n{code}");
    assert!(code.contains("kani::assert"), "V2 gate: kani::assert:\n{code}");
    // No Loom operators in harness
    assert!(!code.contains(" and "), "V2 gate: no `and` operator:\n{code}");
