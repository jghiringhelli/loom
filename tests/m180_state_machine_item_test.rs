...
fn t1_state_machine_generates_enum() {
    let out = ok("module M\n  state_machine Order\n  end\nend\n");
    assert!(
        out.contains("enum OrderState"),
        "expected OrderState enum, got:\n{}",
        out
    );
}
...
fn t2_state_machine_generates_struct() {
    let out = ok("module M\n  state_machine Order\n  end\nend\n");
    assert!(
        out.contains("struct OrderMachine"),
        "expected OrderMachine struct, got:\n{}",
        out
    );
}
...
fn t4_state_machine_has_current_method() {
    let out = ok("module M\n  state_machine Order\n  end\nend\n");
    assert!(
        out.contains("fn current"),
        "expected fn current, got:\n{}",
        out
    );
}
...
fn t5_state_machine_has_transition_method() {
    let out = ok("module M\n  state_machine Order\n  end\nend\n");
    assert!(
        out.contains("fn transition"),
        "expected fn transition, got:\n{}",
        out
    );
}
...
fn t7_state_machine_name_embedded() {
    ...
    assert!(
        out.contains("CheckoutState"),
        "expected CheckoutState, got:\n{}",
        out
    );
    assert!(
        out.contains("CheckoutMachine"),
        "expected CheckoutMachine, got:\n{}",
        out
    );
}
...
fn t9_multiple_state_machines() {
    let out = ok("module M\n  state_machine A\n  end\n  state_machine B\n  end\nend\n");
    assert!(
        out.contains("AState") && out.contains("AMachine"),
        "expected A FSM, got:\n{}",
        out
    );
    assert!(
        out.contains("BState") && out.contains("BMachine"),
        "expected B FSM, got:\n{}",
        out
    );
}
...
fn t11_state_machine_with_other_items() {
    ...
    let out = ok(src);
    assert!(
        out.contains("WorkflowMachine"),
        "expected WorkflowMachine, got:\n{}",
        out
    );
