...
fn parse_demo(name: &str) -> loom::ast::Module {
    ...
    Parser::new(&tokens).parse_module().unwrap_or_else(|e| panic!("{name}: parse error: {e:?}"))
}
...
fn count_disciplines(module: &loom::ast::Module, kind: DisciplineKind) -> usize {
    module.items.iter().filter(|i| {
        matches!(i, Item::Discipline(d) if d.kind == kind)
    }).count()
}

fn count_messaging(module: &loom::ast::Module) -> usize {
    module.items.iter().filter(|i| matches!(i, Item::MessagingPrimitive(_))).count()
}

fn count_telos_fns(module: &loom::ast::Module) -> usize {
    module.items.iter().filter(|i| matches!(i, Item::TelosFunction(_))).count()
}
...
fn m146_order_system_has_telos_function() {
    let m = parse_demo("m146_order_system.loom");
    assert!(count_telos_fns(&m) >= 1, "expected order_fulfillment_alignment telos_function");
}

#[test]
fn m146_order_system_has_three_messaging_channels() {
    let m = parse_demo("m146_order_system.loom");
    assert!(count_messaging(&m) >= 3, "expected RequestResponse + Stream + PointToPoint channels");
}

#[test]
fn m146_order_system_has_cqrs_eventsourcing_cb_saga() {
    let m = parse_demo("m146_order_system.loom");
    assert!(count_disciplines(&m, DisciplineKind::Cqrs) >= 1, "missing CQRS");
    assert!(count_disciplines(&m, DisciplineKind::EventSourcing) >= 1, "missing EventSourcing");
    assert!(count_disciplines(&m, DisciplineKind::CircuitBreaker) >= 1, "missing CircuitBreaker");
    assert!(count_disciplines(&m, DisciplineKind::Saga) >= 1, "missing Saga");
}

#[test]
fn m146_order_system_checker_coherent() {
    let m = parse_demo("m146_order_system.loom");
    let errors: Vec<_> = checker_errors(&m).into_iter()
        .filter(|e| e.contains("implies") && e.contains("CQRS"))
        .collect();
    assert!(errors.is_empty(), "EventSourcing+CQRS should be coherent: {errors:?}");
}
...
fn m146_order_system_emits_rust() {
    ...
    assert!(code.contains("fn ") || code.contains("trait ") || code.contains("struct "),
...
fn m147_evolving_trader_has_all_three_patterns() {
    let m = parse_demo("m147_evolving_trader.loom");
    assert!(has_messaging_pattern(&m, MessagingPattern::Stream),
        "missing Stream (MarketTickStream)");
    assert!(has_messaging_pattern(&m, MessagingPattern::RequestResponse),
        "missing RequestResponse (OrderExecutionChannel)");
    assert!(has_messaging_pattern(&m, MessagingPattern::PointToPoint),
...
fn m147_evolving_trader_has_circuit_breaker_di_uow() {
    let m = parse_demo("m147_evolving_trader.loom");
    assert!(count_disciplines(&m, DisciplineKind::CircuitBreaker) >= 1, "missing CircuitBreaker");
    assert!(count_disciplines(&m, DisciplineKind::DependencyInjection) >= 1, "missing DI");
    assert!(count_disciplines(&m, DisciplineKind::UnitOfWork) >= 1, "missing UnitOfWork");
}
...
fn m148_bioiso_upgraded_has_two_telos_functions() {
    let m = parse_demo("m148_bioiso_upgraded.loom");
    assert!(count_telos_fns(&m) >= 2,
        "expected survival_alignment + selection_pressure_alignment, got {}", count_telos_fns(&m));
}
...
fn m148_bioiso_upgraded_has_two_stream_channels() {
    ...
    assert!(streams >= 2, "expected ChemicalSignalChannel + DivisionEventBus, got {streams}");
}
...
fn m148_bioiso_upgraded_es_and_cqrs_coherent() {
    ...
    let errors: Vec<_> = checker_errors(&m).into_iter()
        .filter(|e| e.contains("implies") && e.contains("CQRS"))
        .collect();
    assert!(errors.is_empty(), "CQRS+EventSourcing pair should be coherent: {errors:?}");
}

#[test]
fn m148_bioiso_upgraded_emits_event_sourcing() {
    let code = emit_demo("m148_bioiso_upgraded.loom");
    assert!(code.contains("Event") || code.contains("event"),
...
fn m149_compiler_self_desc_has_di_and_cb() {
    let m = parse_demo("m149_compiler_self_desc.loom");
    assert!(count_disciplines(&m, DisciplineKind::DependencyInjection) >= 1, "missing DI");
    assert!(count_disciplines(&m, DisciplineKind::CircuitBreaker) >= 1, "missing CircuitBreaker for SMT");
}

#[test]
fn m149_compiler_self_desc_emits_di_container() {
    let code = emit_demo("m149_compiler_self_desc.loom");
    assert!(code.contains("Container") || code.contains("resolve") || code.contains("container"),
...
fn m149_compiler_self_desc_emits_unit_of_work() {
    let code = emit_demo("m149_compiler_self_desc.loom");
    assert!(code.contains("commit") || code.contains("rollback") || code.contains("UnitOfWork"),
...
fn m150_full_composition_has_all_six_discipline_kinds() {
    ...
    assert!(count_disciplines(&m, DisciplineKind::EventSourcing) >= 1, "missing EventSourcing");
    assert!(count_disciplines(&m, DisciplineKind::DependencyInjection) >= 1, "missing DI");
    assert!(count_disciplines(&m, DisciplineKind::CircuitBreaker) >= 1, "missing CB");
    assert!(count_disciplines(&m, DisciplineKind::Saga) >= 1, "missing Saga");
    assert!(count_disciplines(&m, DisciplineKind::UnitOfWork) >= 1, "missing UoW");
}

#[test]
fn m150_full_composition_has_all_three_messaging_patterns() {
    let m = parse_demo("m150_full_composition.loom");
    assert!(has_messaging_pattern(&m, MessagingPattern::Stream), "missing Stream");
    assert!(has_messaging_pattern(&m, MessagingPattern::RequestResponse), "missing RequestResponse");
    assert!(has_messaging_pattern(&m, MessagingPattern::PointToPoint), "missing PointToPoint");
}

#[test]
fn m150_full_composition_has_telos_function() {
    let m = parse_demo("m150_full_composition.loom");
    assert!(count_telos_fns(&m) >= 1, "expected pipeline_health_alignment");
}

#[test]
fn m150_full_composition_checker_coherent() {
    let m = parse_demo("m150_full_composition.loom");
    let cqrs_errors: Vec<_> = checker_errors(&m).into_iter()
        .filter(|e| e.contains("implies") && e.contains("CQRS"))
        .collect();
    assert!(cqrs_errors.is_empty(),
...
fn m150_full_composition_telos_fn_has_all_four_thresholds() {
    let m = parse_demo("m150_full_composition.loom");
    let tf = m.items.iter().find_map(|i| {
        if let Item::TelosFunction(tf) = i { Some(tf) } else { None }
    ...
    assert!(t.propagation.is_some(), "propagation threshold should be set");
}
