...
"#);
    assert!(out.contains("pub capacity: usize"), "missing capacity\n{out}");
}
...
"#);
    assert!(out.contains("capacity: 10"), "default capacity should be 10\n{out}");
}
...
"#);
    assert!(out.contains("pub fn acquire(&mut self) -> Option<T>"), "missing acquire()\n{out}");
}
...
"#);
    assert!(out.contains("pub fn release(&mut self, _item: T)"), "missing release()\n{out}");
}
...
"#);
    assert!(out.contains("LOOM[pool:performance]"), "missing audit comment\n{out}");
...
"#);
    assert!(out.contains("ConnectionPoolPool"), "missing connection\n{out}");
...
"#);
    assert!(out.contains("todo!(\"implement pool acquire\")"), "missing acquire todo\n{out}");
}
