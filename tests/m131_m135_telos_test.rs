...
fn check_telos(src: &str) -> Vec<String> {
    use loom::checker::{TelosFunctionChecker, LoomChecker};
...
"#);
    assert!(out.contains("PnlAlignmentMetric"), "should emit trait: {}", out);
    assert!(out.contains("PnlAlignmentEvaluation"), "should emit evaluation struct: {}", out);
    assert!(out.contains("PnlAlignmentConvergenceTracker"), "should emit tracker: {}", out);
    assert!(out.contains("PnlAlignmentSignalAttention"), "should emit signal attention: {}", out);
}
...
"#);
    assert!(out.contains("fn score(&self) -> f64"), "missing score method: {}", out);
    assert!(out.contains("fn converged(&self) -> bool"), "missing converged method: {}", out);
    assert!(out.contains("fn degraded(&self) -> bool"), "missing degraded method: {}", out);
}
...
"#);
    assert!(out.contains("pub score: f64"), "missing score field: {}", out);
    assert!(out.contains("pub converged: bool"), "missing converged field: {}", out);
    assert!(out.contains("pub degraded: bool"), "missing degraded field: {}", out);
    assert!(out.contains("pub timestamp: u64"), "missing timestamp field: {}", out);
}
...
"#);
    assert!(out.contains("CoherenceDriveMetricFn"), "should emit type alias: {}", out);
    assert!(out.contains("Box<dyn Fn("), "should be a fn pointer type: {}", out);
...
"#);
    assert!(!out.contains("MetricFn"), "no alias without measured_by: {}", out);
}
...
"#);
    assert!(out.contains("0.8500_f64"), "should embed convergence constant: {}", out);
    assert!(out.contains("0.3000_f64"), "should embed divergence constant: {}", out);
    assert!(out.contains("Some(0.5500_f64)"), "should embed warning: {}", out);
    assert!(out.contains("Some(0.9000_f64)"), "should embed propagation: {}", out);
}
...
"#);
    assert!(out.contains("fn is_converging(&self"), "missing is_converging: {}", out);
    assert!(out.contains("fn is_degraded(&self"), "missing is_degraded: {}", out);
    assert!(out.contains("fn eligible_for_propagation(&self)"), "missing propagation check: {}", out);
}
...
"#);
    assert!(out.contains("TODO"), "stubs should emit TODO hints: {}", out);
}
...
"#);
    assert!(out.contains("\"signal_attention\""), "should insert signal_attention axis: {}", out);
    assert!(out.contains("\"risk_selection\""), "should insert risk_selection axis: {}", out);
}
...
"#);
    assert!(out.contains("LOOM[telos:guide]"), "missing guide annotation: {}", out);
    assert!(out.contains("experiment_selection"), "missing experiment_selection axis: {}", out);
    assert!(out.contains("signal_attention"), "missing signal_attention axis: {}", out);
}
...
"#);
    ...
    assert!(errors.iter().any(|e| e.contains("measured_by")), "error should mention measured_by: {:?}", errors);
}
...
"#);
    ...
    assert!(errors.iter().any(|e| e.contains("guides")), "error should mention guides: {:?}", errors);
}
...
"#);
    assert!(errors.is_empty(), "fully specified telos should pass: {:?}", errors);
}
...
"#);
    ...
    assert!(
        errors.iter().any(|e| e.contains("convergence") && e.contains("divergence")),
        "error should mention convergence and divergence: {:?}", errors
    );
}
...
"#);
    ...
    assert!(!errors.is_empty(), "warning above convergence should be rejected");
    assert!(errors.iter().any(|e| e.contains("warning")), "error should mention warning: {:?}", errors);
}
...
"#);
    assert!(!errors.is_empty(), "propagation below convergence should be rejected");
    assert!(errors.iter().any(|e| e.contains("propagation")), "error should mention propagation: {:?}", errors);
}
...
"#);
    assert!(errors.is_empty(), "valid thresholds should pass: {:?}", errors);
}
...
"#);
    assert!(out.contains("maintain systemic equilibrium"), "statement should appear in header: {}", out);
    assert!(out.contains("LOOM[telos_fn]"), "should include LOOM marker: {}", out);
}
...
"#);
    ...
    assert!(out.contains("PnlRiskAdjustedScore"), "should convert to PascalCase: {}", out);
}
