...
impl SelfCertChecker {
    pub fn new() -> Self { SelfCertChecker }
    ...
    fn check_certificate(&self, cert: &CertificateDef, errors: &mut Vec<LoomError>) {
        let valid_values = ["proven", "verified", "partial", "assumed", "checked", "passed"];
        let known_fields = ["type_safety", "memory_safety", "termination", "purity",
                            "effect_safety", "privacy", "timing_safety", "separation"];
        for field in &cert.fields {
            ...
            if known_fields.contains(&field.name.as_str()) && !valid_values.contains(&field.value.as_str()) {
                errors.push(LoomError::TypeError {
                    msg: format!(
