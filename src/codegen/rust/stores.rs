...
impl RustEmitter {
    ...
    fn store_header(&self, store: &StoreDef, out: &mut String) {
        ...
        if !store.config.is_empty() {
            let cfg: Vec<String> = store.config.iter()
    ...
    fn emit_crud_trait_impl(&self, store_name: &str, table: &str, pk_field: &str, out: &mut String) {
        ...
        out.push_str(&format!(
            "pub trait {table}Repository {{\n"
        ));
        out.push_str(&format!(
            "    /// Find by primary key `{pk_field}`.\n"
        ));
        out.push_str(&format!(
    ...
    fn codegen_keyvalue_store(&self, store: &StoreDef, out: &mut String) {
        ...
        let key_type = store.schema.iter().find_map(|e| {
            if let StoreSchemaEntry::KeyType { ty, .. } = e { Some(self.emit_type_expr(ty)) } else { None }
        }).unwrap_or_else(|| "String".to_string());

        let value_type = store.schema.iter().find_map(|e| {
            if let StoreSchemaEntry::ValueType { ty, .. } = e { Some(self.emit_type_expr(ty)) } else { None }
        ...
        out.push_str(&format!("    fn get(&self, key: &{key_type}) -> Option<{value_type}>;\n"));
        out.push_str(&format!("    fn put(&self, key: {key_type}, value: {value_type}) -> Result<(), String>;\n"));
        out.push_str(&format!("    fn del(&self, key: &{key_type}) -> Result<(), String>;\n"));
        out.push_str("}\n\n");
    }
    ...
    fn codegen_document_store(&self, store: &StoreDef, out: &mut String) {
        ...
        out.push_str("// LOOM[store:Document]: schema-optional collections → typed Rust structs\n\n");
        for entry in &store.schema {
    ...
    fn codegen_hypercube_store(&self, store: &StoreDef, out: &mut String) {
        ...
        out.push_str("// LOOM[store:Hypercube]: MOLAP axes → dimension types; measures → fact structs\n\n");
        for entry in &store.schema {
            match entry {
                StoreSchemaEntry::DimensionEntry { name, fields, .. } => {
    ...
    fn codegen_graph_store(&self, store: &StoreDef, out: &mut String) {
        ...
        for entry in &store.schema {
            match entry {
                ...
                StoreSchemaEntry::Edge { name, source, target, fields, .. } => {
                    out.push_str(&format!("// Graph edge: {} ({} → {})\n", name, source, target));
                    self.emit_named_struct(name, fields, out);
        ...
        if !node_types.is_empty() {
            ...
            let edges = if edge_types.is_empty() { "()".to_string() } else { edge_types.join(" | ") };
            out.push_str(&format!(
    ...
    fn codegen_timeseries_store(&self, store: &StoreDef, out: &mut String) {
        ...
        out.push_str("// LOOM[store:TimeSeries]: events have mandatory timestamp; ordered by time\n\n");
        ...
        for entry in &store.schema {
            if let StoreSchemaEntry::Event { name, fields, .. } = entry {
                ...
                let has_ts = fields.iter().any(|f| f.name == "timestamp" || f.name == "ts");
                out.push_str(STORE_DERIVES);
                out.push('\n');
                out.push_str(&format!("pub struct {} {{\n", name));
                if !has_ts {
                    out.push_str("    pub timestamp: i64, // LOOM[ts]: Unix nanos — auto-injected\n");
                }
                self.emit_struct_fields(fields, out);
                out.push_str("}\n\n");
            }
        ...
        let retention = store.config.iter()
    ...
    fn codegen_vector_store(&self, store: &StoreDef, out: &mut String) {
        ...
        out.push_str("// LOOM[store:Vector]: fixed-dimension embeddings with similarity search contract\n\n");

        let dimension = store.config.iter()
    ...
    fn codegen_inmemory_store(&self, store: &StoreDef, inner: &Box<StoreKind>, out: &mut String) {
        ...
        let capacity = store.config.iter()
            .find(|c| c.key == "capacity" || c.key == "max_size")
            .map(|c| c.value.as_str())
            .unwrap_or("unbounded");

        out.push_str(&format!(
            "// LOOM[cache:capacity]: {capacity}\n"
        ));
    ...
    fn codegen_flatfile_store(&self, store: &StoreDef, out: &mut String) {
        ...
        out.push_str("// LOOM[store:FlatFile]: columnar row struct for Parquet/CSV/HDF5 serialization\n\n");

        let format = store.config.iter()
            .find(|c| c.key == "format")
            .map(|c| c.value.as_str())
            .unwrap_or("Parquet");
    ...
    fn codegen_distributedlog_store(&self, store: &StoreDef, out: &mut String) {
        ...
        let partitions = store.config.iter()
            .find(|c| c.key == "partitions")
            .map(|c| c.value.as_str())
            .unwrap_or("1");
        let replication = store.config.iter()
        ...
        out.push_str(&format!("    fn produce(&self, msg: {msg_type}) -> Result<u64, String>;\n"));
        out.push_str("}\n\n");
