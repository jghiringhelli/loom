...
impl RustEmitter {
    ...
    pub fn emit(&self, module: &Module) -> String {
        ...
        // V7: dynamic audit header — honest record of what's proved vs. declared-only.
...
fn emit_audit_header(module: &Module) -> String {
    ...
    for item in &module.items {
        match item {
            Item::Fn(fd) => {
                ...
                if fd.stochastic_process.is_some() { stochastic    += 1; }
                if fd.distribution.is_some()        { distributions += 1; }
                if fd.separation.is_some()           { separation    += 1; }
                if fd.timing_safety.is_some()        { timing        += 1; }
                if fd.termination.is_some()          { termination   += 1; }
            }
    ...
    if separation > 0 {
        declared_only.push(format!("separation-logic ({separation} fn — Prusti for full proof)"));
    }
    if timing > 0 {
        declared_only.push(format!("timing-safety ({timing} fn — ctgrind/dudect for full proof)"));
    }
    if termination > 0 {
        declared_only.push(format!("termination ({termination} fn — cargo kani / Dafny for proof)"));
    }
    ...
    h.push_str("// LOOM[v7:audit]: do not edit manually. Each LOOM[...] comment records a decision.\n");
    h.push('\n');
    h
}
