...
| **Dependent types (size bounds)** | `proposition: vec.len() = n` | compile-time phantom size check | `dependent_test` (6 tests) + `experiments/proofs/dependent-types/proof.dfy` | 🔴 formal (Dafny 4.11 — 7 verified, 0 errors) | [`typenum`](https://crates.io/crates/typenum) [`generic-array`](https://crates.io/crates/generic-array) | — |
